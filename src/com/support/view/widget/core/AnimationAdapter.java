...
public class AnimationAdapter extends BaseAdapterDecorator {
    ...
    float lastMovingProgress;

    protected void notify(float progress) {
        ...
        int number = lastVisiblePosition - firstVisiblePosition;

        fistItemSlope = 1f / factor;
        itemsSlopeStep = (fistItemSlope - 1f) / number;
        if (lastMovingProgress > 0.99 && progress == 0) {
            progress = 1f;
        }
        float y;
        for (int i = 0; i <= number; i++) {
            View itemView = mListViewWrapper.getChildAt(i);

            y = Math.min(1f, (fistItemSlope - itemsSlopeStep * i) * progress);
            if (itemView != null) {
                itemView.setTranslationX(
                        mListViewWrapper.getWidth() - mListViewWrapper.getWidth() * y);
            }
        }
